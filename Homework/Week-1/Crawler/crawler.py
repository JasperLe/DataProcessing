...
def scrape_top_250(url):
    ...
    for b in soup.findAll("td"):
        for link in b.findAll("a"):
            a = imdb + link.get("href")
            movie_urls.append(a)
        # return the list of URLs of each movie's page on IMDB
    movie_urls = movie_urls[:-8]
    movie_urls = movie_urls[1::2]
    print a
    return movie_urls
...
def scrape_movie_page(dom):
    ...
    genres = ""
    actors = ""
    title = dom.by_class("title_wrapper")[0].by_tag("h1")[0].content.encode("ascii", "ignore")
    title = title.split("&n")[0]
    duration = dom.by_tag("time")[0].content.encode("ascii", "ignore")
    rating = dom.by_class("ratingValue")[0].by_tag("span")[0].content.encode("ascii", "ignore")
    while True:
        try:
            a = dom.by_class("subtext")[0].by_class("itemprop")[e].content.encode("ascii", "ignore")
            genres = genres + a + ", "
            e += 1
        except IndexError:
            break
    directors = dom.by_class("rec-director rec-ellipsis")[0].content.encode("ascii", "ignore")
    writers = dom.by_class("credit_summary_item")[0].by_class("itemprop")[0].content.encode("ascii", "ignore")
    e = 0
    while True:
        try:
            a = dom.by_class("credit_summary_item")[2].by_class("itemprop")[e].content.encode("ascii", "ignore")
            actors = actors + a + ", "
            e += 1
            print a
        except IndexError:
    ...
    print title, duration, genres, directors, writers, actors, rating, \
        n_ratings
