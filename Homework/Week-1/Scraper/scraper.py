...
def repeater(rep):
    return int(rep.by_class("lister-current-last-item")[0].content)
...
def get_actors(a, e):
    ...
    while True:
        try:
            ...
            break
    actor_list = actor_list[:-2].strip()
    return actor_list

# dom and repeat as input, list of runtime on dom as output
def get_runtime(a, e):
    b = a.by_class("lister-item-content")[e].by_class("runtime")[0].content.encode("ascii", "ignore")
    b = b[:-4]
    return b
